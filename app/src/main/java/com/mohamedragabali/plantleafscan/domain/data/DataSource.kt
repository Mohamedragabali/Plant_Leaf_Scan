...
import com.mohamedragabali.plantleafscan.domain.PlantNameDetails
...
object DataSource {
    ...
    var currentPlantName = plantName()
    fun pressPlantNameAtIndex(position: Int ) {
       currentPlantName=  currentPlantName.mapIndexed { index, plantNameDetails ->
            if (index == position) {
                PlantNameDetails(plantNameDetails.name, true)
            } else {
                PlantNameDetails(plantNameDetails.name ,false)
            }
        }
    }


private fun plantName(): List<PlantNameDetails> = allDisease()
...
private fun allDisease() = listOf(
    ...
    Pair(R.drawable.cherry_including_sour_powdery_mildew, "Cherry (including_sour) Powdery mildew"),
    ...
    Pair(R.drawable.orange_haunglongbing_citrus_greening, "Orange Haunglongbing (Citrus_greening)"),
    ...
    Pair(R.drawable.tomato_spider_mites_two_spotted_spider_mite, "Tomato Spider mites Two spotted spider mite"),
    ...
    fun  diseaseTreatment(diseaseName:String ) = listOf(
        ...
        Pair("Grape Leaf blight (Isariopsis Leaf Spot)", "Apply fungicides and improve air circulation."),
        ...
        Pair("Tomato Spider mites Two spotted spider mite", "Use miticides and ensure proper watering."),
